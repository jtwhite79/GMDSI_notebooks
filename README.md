...
This repo uses [pixi](https://pixi.sh) to manage the python environment *and* the
MODFLOW/PEST++ executables. You do not need conda, and you do not need to install
python yourself.

 - Install pixi by following the directions here: [pixi installation](https://pixi.sh/latest/#installation).
 - Open a terminal (on __Windows__, use PowerShell or the Command Prompt) and navigate
   to the course repo folder on your machine. You can do this by typing `cd *your folder path*`
   and pressing < enter >. Replace *your folder path* with the path to the course
   material folder on your computer.
 - Type `pixi install`. This creates a self-contained environment inside the repo
   (in a `.pixi` folder) with everything the notebooks need. It may take a while.
   You can inspect *pixi.toml* to see what is being installed.
 - Then type `pixi run get-exes`. This downloads MODFLOW 6, MODPATH 7, ZONEBUDGET 6
   and the PEST++ suite into a `bin` folder in the repo. Nothing is installed
   system-wide, and the binaries are not stored in the repo itself.

You can check that the executables arrived with `pixi run check-exes`.

**Start jupyter**
You will need to do this step any time you wish to open one of the course notebooks.
From the course repo folder, type:
